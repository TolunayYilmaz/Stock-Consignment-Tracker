...
def _build_reset_html(to_email: str, link: str) -> str:
    return f"""\
<!DOCTYPE html>
<html lang="tr">
  <body style="margin:0;padding:0;background-color:#f5f5dc;font-family:Arial,Helvetica,sans-serif;">
        ...
        <td align="center" style="padding:32px 16px;">
          <table role="presentation" width="480" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="border-radius:16px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.08);">
            <tr>
              <td style="background-color:#15803d;padding:24px 32px;text-align:center;">
                <h1 style="margin:0;color:#ffffff;font-size:22px;">Stok Emanet</h1>
                <p style="margin:6px 0 0;color:#d1fae5;font-size:13px;">Şifre Sıfırlama</p>
              </td>
            </tr>
            <tr>
              <td style="padding:32px;">
                ...
                <p style="margin:0 0 24px;color:#44403c;font-size:15px;line-height:1.6;">
                  Şifrenizi sıfırlamak için aşağıdaki butona tıklayın. Bu link 15 dakika geçerlidir.
                </p>
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td align="center" style="padding:8px 0 24px;">
...
def send_password_reset_email(to_email: str, token: str, base_url: str = "http://localhost:8000") -> bool:
    """Resend üzerinden şifre sıfırlama maili gönderir. Hata olursa loglayıp False döner."""
    if not RESEND_API_KEY:
        print("[email] RESEND_API_KEY is not set; skipping send")
        return False
    import resend
        ...
        resend.Emails.send(
            {
                "from": FROM_EMAIL,
                "to": [to_email],
                "subject": "Stok Emanet - Şifre Sıfırlama",
                "html": _build_reset_html(to_email, reset_link),
            }
        )
        return True
    except Exception as e:
        print(f"[email] FAILED to send reset to {to_email}: {type(e).__name__}: {e}")
        return False